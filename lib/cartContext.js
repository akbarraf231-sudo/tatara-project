...
export function CartProvider({ children }) {
  ...
  const addItem = useCallback((product, options = {}) => {
    ...
    const sizeUnits = Math.max(1, Number(options.sizeUnits) || 1);
  ...
  return (
    <CartContext.Provider
      value={{ items, addItem, removeItem, updateQty, clearCart, total, hasSpecialItems, isOpen, openCart, closeCart }}
    >
      {children}
    </CartContext.Provider>
  );
