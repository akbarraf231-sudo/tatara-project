    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div className="bg-[#fce8e2] rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`p-4 rounded-2xl border-2 transition-all ${
                  paymentMethod === 'qris'
                    ? 'border-[#5a1f2a] bg-white shadow-md'
                    : 'border-[#e3b9b9] bg-white hover:bg-[#e3b9b9]/30'
                }`}
              >
          ...
          {paymentMethod === 'qris' && (
            <div className="bg-white rounded-2xl p-4 border-2 border-[#e3b9b9]">
              <p className="text-sm text-[#5a1f2a] font-semibold mb-3 text-center">
          ...
          {paymentMethod === 'cash' && (
            <div className="bg-white rounded-2xl p-4 border-2 border-[#e3b9b9]">
              ...
              <p className="text-sm text-[#722f37] mb-3">
                Silakan bayar <span className="font-bold text-[#5a1f2a]">Rp {total.toLocaleString('id-ID')}</span> saat pickup di toko.
              </p>
              {settings?.location_link && (
                <a
                  ...
                  className="inline-flex items-center gap-1 text-[#5a1f2a] hover:text-[#722f37] font-semibold text-sm"
                >
                  📍 Lihat Lokasi Toko
                </a>
              )}
          ...
          {!loadingSettings && !settings?.whatsapp_number && (
            <div className="bg-yellow-50 border-2 border-yellow-200 rounded-2xl p-3 text-sm text-yellow-800">
              ⚠️ Admin belum setup WhatsApp number.
            </div>
          )}
          ...
          <button
            onClick={onClose}
            className="w-full bg-[#e3b9b9] hover:bg-[#c89292] text-[#5a1f2a] hover:text-white font-semibold py-2 px-4 rounded-full transition-colors"
          >
            Tutup
          </button>
        </div>
